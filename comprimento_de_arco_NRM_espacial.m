function[LAMBDA,u,DELTA_U,DELTA_L,k_o,CST,graph1,DELTA_U_ANTERIOR,fcc_i_anterior,LAMBDA_ANTERIOR] = comprimento_de_arco_NRM_espacial(n,imax,TOL,Kcc,fcc,k_o,DELTA_L,DELTA_L_0,DELTA_L_MAX,DELTA_L_MIN,DELTA_U,LAMBDA,u,comp_arco_variavel,Nd,zeta,GDL,NELEM,CONEC,COORDNOS,PROPELEM,tipo_deformacao,tipo_linear_fisico,SIGMAANT,sigma,normal,NNOSREST,REST,graph1,DELTA_U_ANTERIOR,fcc_i_anterior,LAMBDA_ANTERIOR)
    ...
    DELTA_U = DELTA_U_1;
    [fcc_i] = forcas_internas_espacial(GDL,NELEM,CONEC,COORDNOS,(u+DELTA_U),PROPELEM,tipo_deformacao,tipo_linear_fisico,SIGMAANT,sigma,normal,n,NNOSREST,REST);
    ...
    if (n > 1 && ACO_F > TOL*sqrt(((LAMBDA)^2)*(fcc'*fcc))) || sum(isnan(Erro))
        fprintf('\nNão convergiu no incremento %i',n);
        fprintf('\nLAMBDA = %.2f',LAMBDA);
        fprintf('\nResíduo = %.4f ', ACO_F);
        fprintf('\nDeslocamento = %.6f ',u(39));
        ...
        if i == ACO_F > TOL*sqrt(((LAMBDA)^2)*(fcc'*fcc))
            fprintf('\nNão foi possível a correção no incremento %i',n);
            fprintf('\nTentar com outros parâmetros de comprimento de arco');
            fprintf('\nResíduo = %.4f \n', ACO_F);
            
            figure(1);
            scatter(-graph1(n,3),graph1(n,2),'o');
        end
