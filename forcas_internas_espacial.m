function[fcc_i] = forcas_internas_espacial(GDL,NELEM,CONEC,COORDNOS,deslocamentos,PROPELEM,tipo_deformacao,tipo_linear_fisico,SIGMAANT,sigma,normal,n,NNOSREST,REST)
    ...
    for k = 1: +1 : NELEM 
        ...
        dxf = COORDNOS(nof,1) - COORDNOS(noi,1) + deslocamentos(3*nof-2) - deslocamentos(3*noi-2);       % Deltax do elemento (seu comprimento na direção x);
        dyf = COORDNOS(nof,2) - COORDNOS(noi,2) + deslocamentos(3*nof-1) - deslocamentos(3*noi-1);       % Deltay do elemento (seu comprimento na direção y);
        dzf = COORDNOS(nof,3) - COORDNOS(noi,3) + deslocamentos(3*nof) - deslocamentos(3*noi);           % Deltaz do elemento (seu comprimento na direção z);
